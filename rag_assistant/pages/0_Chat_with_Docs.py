...
SESSION_IDS_FILE = "session_ids.pkl"
...
def handle_assistant_response(user_query):
    ...
    with ((st.chat_message("assistant"))):
        ...
        ai_response = ""
        with tracing_v2_enabled(project_name="Chat with Docs", tags=["LangChain", "Chain", "Chat History"]):
        ...
        save_chat_history(session_id, msgs)
...
def main():
    ...
    # Repopulate chat_histories with loaded session IDs
    for sid in session_ids:
        if sid not in st.session_state.chat_histories:
            st.session_state.chat_histories[sid] = get_chat_history(sid)
    ...
    if st.sidebar.button("New Chat Session"):
        ...
        save_session_ids(chat_sessions)

    selected_session = st.sidebar.radio("Select Chat Session", chat_sessions)
